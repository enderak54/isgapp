...
# Git durumu (MSYS yol çevirmesi git için kapatılır yoksa /c/ yolu Windows git tarafından anlaşılmaz)
if [ -d "$REPO_ROOT/.git" ]; then
    if ! env -u MSYS_NO_PATHCONV MSYS2_ARG_CONV_EXCL="" git -C "$REPO_ROOT" diff --quiet 2>/dev/null; then
        warn "Repo'da commitlenmemiş değişiklikler var — güncelleme öncesi commit/stash önerilir"
        env -u MSYS_NO_PATHCONV MSYS2_ARG_CONV_EXCL="" git -C "$REPO_ROOT" status --short 2>/dev/null | head -10 | while read -r line; do warn "  $line"; done
    fi
fi
...
if [ "$DRY_RUN" = "1" ]; then
    ...
    env -u MSYS_NO_PATHCONV MSYS2_ARG_CONV_EXCL="" git -C "$REPO_ROOT" fetch --dry-run 2>&1 | head -20 || true
    LOCAL_HEAD=$(env -u MSYS_NO_PATHCONV MSYS2_ARG_CONV_EXCL="" git -C "$REPO_ROOT" rev-parse --short HEAD 2>/dev/null || echo "?")
    REMOTE_HEAD=$(env -u MSYS_NO_PATHCONV MSYS2_ARG_CONV_EXCL="" git -C "$REPO_ROOT" rev-parse --short origin/main 2>/dev/null || echo "?")
    log "[DRY-RUN] Lokal HEAD: $LOCAL_HEAD -> Uzak HEAD: $REMOTE_HEAD"
else
    env -u MSYS_NO_PATHCONV MSYS2_ARG_CONV_EXCL="" git -C "$REPO_ROOT" pull --ff-only
fi
