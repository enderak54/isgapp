...
CREATE TABLE public.personel_belgeleri (
    ...
    CONSTRAINT personel_belgeleri_belge_tipi_check CHECK ((belge_tipi = ANY (ARRAY['isg_egitim'::text, 'yuksekte_calisma'::text, 'myk'::text, 'operator_belgesi'::text, 'kkd'::text, 'oryantasyon'::text, 'saglik_raporu'::text, 'sertifika'::text, 'diger'::text, 'diploma'::text])))
);
...
-- NOT: Storage ayarları (bucket'lar, storage.objects policy'leri,
-- storage grant'ları) BURADA DEĞİL — storage.buckets/objects
-- tabloları db init sırasında HENÜZ YOKTUR; bunları storage-api
-- servisi ayağa kalkınca oluşturur. Bu yüzden o kısım ayrı dosyada:
--   self-host/storage-setup.sql
-- (setup.sh, stack ayağa kalktıktan sonra uygular)

-- public şema grant'ları
